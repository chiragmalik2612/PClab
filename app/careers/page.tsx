...
export default function CareersPage() {
  const [activeTab, setActiveTab] = useState<'positions' | 'collaborations'>('positions');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 md:py-16 w-full min-h-[60vh]">
      ...
      <div className="border-b border-slate-200 pb-6 mb-8">
        ...
        <p className="text-slate-600 mt-4 text-base md:text-lg leading-relaxed w-full">
      ...
      {activeTab === 'positions' && (
        <div className="w-full">
          <div className="mb-6">
            <h2 className="text-2xl font-bold text-slate-900 tracking-tight">Current Openings</h2>
            <p className="text-slate-500 text-sm mt-1">Explore available roles or check back periodically for updates.</p>
          </div>
          ...
          {careersData.length > 0 ? (
            <div className="space-y-6 w-full">
              {careersData.map((job) => (
                <div 
                  key={job.id} 
                  className="border border-slate-200 p-6 md:p-8 bg-white rounded-sm hover:border-[#009966]/60 hover:shadow-md transition-all duration-300 group flex flex-col w-full"
                >
                  ...
                  <p className="text-slate-600 text-base leading-relaxed mb-6 w-full">
                    {job.description}
                  </p>

                  <div className="bg-slate-50 border border-slate-100 p-5 rounded-sm mb-6 w-full">
                    ...
                    <ul className="space-y-2">
                      {job.requirements.map((req, idx) => (
                        <li key={idx} className="flex items-start text-slate-600 text-sm font-medium">
                          <svg className="w-4 h-4 text-[#009966] mr-3 mt-0.5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="bg-slate-50 border border-slate-200/60 border-dashed rounded-sm py-20 px-6 flex flex-col items-center justify-center text-center w-full">
              <div className="w-12 h-12 bg-white shadow-sm rounded-full flex items-center justify-center mb-4 text-slate-300">
      ...
      {/* Tab 2: Collaborate With Us */}
      {activeTab === 'collaborations' && (
        <div className="w-full bg-slate-50 border border-slate-200 border-l-4 border-l-[#009966] rounded-sm p-8 md:p-10">
          <div className="w-full">
            <h2 className="text-2xl md:text-3xl font-bold text-slate-900 tracking-tight mb-4">
              Collaborate With Us
            </h2>
            <p className="text-slate-600 text-base md:text-lg leading-relaxed mb-6 w-full">
            ...
            <p className="text-slate-600 text-base md:text-lg leading-relaxed mb-8 w-full">
            ...
            <a
              ...
              className="inline-flex items-center gap-2.5 bg-slate-900 hover:bg-[#009966] text-white px-7 py-3.5 rounded-sm font-bold text-sm tracking-wide transition-colors shadow-sm"
            >
              <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 002-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
              </svg>
              Get in Touch ({contactEmail})
            </a>
          </div>
        </div>
      )}
